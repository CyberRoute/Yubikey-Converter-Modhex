...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='YubiKey public identity')
    parser.add_argument('-y', '--yksn', help='Converts the S/N to modhex')
    args = parser.parse_args()
    sn = hex(int(args.yksn)).split('x')[-1]
    conversion = str(hex_to_modhex(sn.encode()))
    conversion = conversion.split('b')[-1]
    print('cccccc' + conversion[1:-1])